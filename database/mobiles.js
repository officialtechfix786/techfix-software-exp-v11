...
const mobilesDatabase = [

    {
        ...
        solutions: [
            ...
            {
                name: "Firmware / Flash",
                description: "Firmware and authorized software support.",
                price: "Contact for Price"
            },
            {
                name: "IMEI Repair",
                description: "Professional repair information.",
                price: "Contact for Price"
            },
            {
                name: "Network Repair",
                description: "Network troubleshooting and repair information.",
                price: "Contact for Price"
            }
        ]
    },

    {
        ...
        solutions: [
            ...
            {
                name: "Flash",
                description: "Firmware flashing information.",
                price: "Contact for Price"
            },
            {
                name: "IMEI Repair",
...

    {
        ...
        solutions: [
            ...
            {
                name: "Flash",
                description: "Authorized firmware flashing information.",
                price: "Contact for Price"
            },
            {
                name: "Network",
                description: "Network troubleshooting information.",
                price: "Contact for Price"
            }
        ]
    },
...

    {
        ...
        solutions: [
            {
                name: "Firmware",
                description: "Official firmware information.",
                price: "Contact for Price"
            },
            {
                name: "Flash",
                description: "Authorized flashing information.",
                price: "Contact for Price"
            }
        ]
    },

    {
        ...
        solutions: [
            ...
            },
            {
                ...
                description: "Professional software information.",
                price: "Contact for Price"
            }
        ]
    },

    {
        ...
        solutions: [
            {
                name: "Restore",
                description: "Official Apple restore information.",
                price: "Contact for Price"
            },
            {
                name: "Recovery",
                description: "Recovery mode support information.",
                price: "Contact for Price"
            },
            {
                name: "DFU",
                description: "DFU mode information.",
                price: "Contact for Price"
            }
        ]
    },

    {
        ...
        solutions: [
            ...
            {
                name: "Recovery",
                description: "Recovery mode information.",
                price: "Contact for Price"
            }
        ]
    }
...
window.searchMobiles = function (query) {
    ...
    return mobilesDatabase.filter(function (mobile) {
        ...
        // Let technicians search shorthand such as "Samsung A15" even when
        // the full stored model name is "Samsung Galaxy A15".
        return search
            .split(/\s+/)
            .every(function (term) {
...
const TECHFIX_BRAND_LOGOS = {
    ...
    POCO: "../assets/images/brands/poco.svg",
    OPPO: "../assets/images/brands/oppo.svg",
    Vivo: "../assets/images/brands/vivo.svg",
    Realme: "../assets/images/brands/realme.svg",
    Tecno: "../assets/images/brands/tecno.svg",
    Infinix: "../assets/images/brands/infinix.svg",
...
/*
 * BUGFIX: the map above only ever covered 11 brands, so every other
 * brand (this file's own "Redmi", plus anything from the wider
 * catalog -- Huawei, Sony, ZTE, Meizu, ...) fell back to the generic
 * TechFix logo instead of its own. tools/generate-brand-logos.py
 * generates a real, correctly-labeled badge for every brand name
 * that shows up anywhere in the catalog, saved as
 * assets/images/brands/<slugified-brand-name>.svg -- so we can
 * resolve any brand's logo deterministically instead of relying on
 * a fixed whitelist.
 */
function slugifyBrandName(value) {
    return String(value || "")
        .trim()
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "-")
...
function resolveBrandLogo(brand) {
    if (TECHFIX_BRAND_LOGOS[brand]) return TECHFIX_BRAND_LOGOS[brand];
    return "../assets/images/brands/" + slugifyBrandName(brand) + ".svg";
}
...
mobilesDatabase.forEach(function (mobile) {
    ...
    mobile.brandLogo = resolveBrandLogo(mobile.brand);

    // Keep the existing card/model-image API while using a valid, lightweight
    // local asset (previously pointed at the 1.6MB techfix-logo.png).
    if (mobile.image === "../assets/images/logo.png") {
        mobile.image = "../assets/images/logo/techfix-logo.svg";
    }

    // ROOT CAUSE FIX: none of the 13 core models ever had an `image` or
    // `image_url` field, so the mobiles.html card template's
    // `(mobile.image || mobile.image_url) ? ... : ""` check always fell
    // through to "" and the device-image block never rendered — that's
    // why mobile pictures stopped showing on the Mobiles page.
    // The CSV catalog merge in search-upgrade.js only fills `image` for
    // items it matches by normalized "brand model" key, and the CSV's
    // model names include suffixes like "(256GB)" that don't match these
    // 13 curated entries, so it never filled the gap either.
    // Fall back to the brand's local SVG logo — it always exists on disk,
    // so every card gets a real, local, non-404 image every time.
    if (!mobile.image && !mobile.image_url) {
        mobile.image = mobile.brandLogo;
    }
