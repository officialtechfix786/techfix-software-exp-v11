...
document.addEventListener("DOMContentLoaded", () => {
    ...
    /* =====================================================
       ...
       Previously every failed <img> (broken remote model
       photo, missing asset, etc.) was swapped to the 1.6MB
       techfix-logo.png. On a catalog/mobiles page with many
       remote images, several failures compounded into
       megabytes of extra, unnecessary downloads. It also
       looked wrong -- a full brand logo standing in for a
       missing phone photo.

       Fallback is now a 4KB SVG placeholder, and remote
       images additionally get a load timeout so a slow/dead
       host can't hang the page waiting for a browser 'error'
       event that may never fire.
       ===================================================== */

    const IMAGE_PLACEHOLDER =
        (location.pathname.includes("/pages/") ? "../" : "") +
        "assets/images/ui/image-placeholder.svg";
    const REMOTE_IMAGE_TIMEOUT_MS = 6000;

    function useFallback(image) {
        if (image.src.includes("image-placeholder.svg")) return;
    ...
    document
        ...
        .forEach((image) => {
            ...
            // Remote (cross-origin) catalog images: if the host is slow or
            // unresponsive, don't let the browser wait on it indefinitely --
            // swap to the placeholder after a short timeout instead.
            const isRemote = /^https?:\/\//i.test(image.getAttribute("src") || "");
            if (isRemote && !image.complete) {
