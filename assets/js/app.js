...
document.addEventListener("DOMContentLoaded", () => {
        ...
        const mobileResults =
            mobiles
                .filter((mobile) => {
                    ...
                    return term.split(/\s+/).every((token) => text.includes(token));
        ...
        searchInput.addEventListener(
            ...
            () => {
                ...
                searchDebounce = window.setTimeout(() => {
                    showResults(searchDatabase(searchInput.value));
                }, 120);
            }
        );
    ...
    const setSearchOpen = (open) => {
        ...
        if (open && searchInput) window.setTimeout(() => searchInput.focus(), 0);
    };
    if (searchPanel) searchPanel.hidden = true;
    if (searchButton) searchButton.addEventListener("click", () => setSearchOpen(true));
    if (closeSearch) closeSearch.addEventListener("click", () => setSearchOpen(false));
        ...
        .forEach((image) => {
            ...
            image.addEventListener(
                ...
                () => {
                    ...
                    const fallback = location.pathname.includes("/pages/")
